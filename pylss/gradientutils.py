...
from math import sqrt, isnan, log, fabs, log10
...
def get_lss_peak_width(c_length, c_particle, logkw, s, init_b, final_b, tg, flow, v_m, v_d, s_extra):
    ...
    t0 = v_m/flow # Fixed: was v_m*flow
    b = (t0*(final_b-init_b)*s)/tg
    k0 = pow(10, logkw-(s*init_b)) # Fixed: was missing pow(10, ...)
    ke = 1/(2.3*b)
    p = (2.3*b*k0)/(k0+1)
    ...
    if final_b > 1 or final_b < 0 or isnan(final_b) or init_b < 0 or init_b > 1 or isnan(init_b) or tg <= 0 or isnan(tg):
        ...
        for row in lssparam:
            ...
            try:
                ...
                # Allow some margin after tg for elution (e.g. 2 * t0)
                if tr_tmp < t0 or tr_tmp > (tg + t0*2) or isnan(tr_tmp):
                    continue
                else:
        ...
        if len(trtab) < len(lssparam):
            ...
            for i in range(1, len(trtab)):
                ...
                if lowestrs is None or rs < lowestrs:
                    lowestrs = rs
            ...
            # Return lowestrs even if no critical pairs are found (lstcc empty)
            return lstcc, lowestrs

def get_lss_gradient_critical_selectivity(c_length, c_particle, init_b, final_b, tg, flow, v_m, v_d, lssparam , crit_alpha=1.1):
    ...
    if final_b > 1 or final_b < 0 or isnan(final_b) or init_b < 0 or init_b > 1 or isnan(init_b) or tg <= 0 or isnan(tg):
        ...
        for row in lssparam:
            ...
            try:
                tr_tmp = lssmol.rtpred(lss_logkw, lss_s, tg, init_b, final_b, t0, td)
                if tr_tmp < t0 or isnan(tr_tmp) or tr_tmp > (tg + t0*2):
                    return None, None 
                else:
        ...
        for i in range(1, len(trtab)):
            ...
            if lowest_alpha is None or alpha < lowest_alpha:
                lowest_alpha = alpha
...
def get_logss_peak_width(c_length, c_particle, lss_logkw, lss_s, init_b, final_b, tg, flow, v_m, v_d, s_extra):
    ...
    t0 = v_m/flow # Fixed: was v_m*flow
    b = lss_s * alpha
    k0 = pow(10, lss_logkw-(lss_s*init_b)) # Fixed: was missing pow(10, ...)
    ke = 1/(2.3*b)
    p = (2.3*b*k0)/(k0+1)
    G = ((1+p+(p**2/3))/(1+p)**2)**(1/2.)
    Nobs = 0.
...
def get_logss_gradient_critical_selectivity(c_length, c_particle, init_b, final_b, tg, flow, v_m, v_d, lssparam, crit_alpha=1.1):
    """Function to estimate critical couples and resolutions for logarithmic gradient mode
       N.B.: The molecule id correspond to the exact position in lssparam.
    """
    if final_b > 1 or final_b < 0 or isnan(final_b) or init_b < 0 or init_b > 1 or isnan(init_b) or tg <= 0 or isnan(tg):
